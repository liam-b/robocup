...
module.exports = function (leds) {
  ...
  console.log('[trace]'.bold() + ' [info]'.blue().bold() + ' [log]'.green().bold() + ' [warn]'.yellow().bold() + ' [error]'.red().bold() + ' function'.purple() + ' value'.cyan());

  this.trace = function (task, text) {
    console.log('[trace]'.bold() + '\x1b[0m ' + getTime().black().bold() + ' ' + task.purple() + ' ' + text);
  }
