...
module.exports = function (leds, traceLevel) {
  this.leds = leds;
  this.cyan = function (text) {
    return '\x1b[36m' + text + '\x1b[0m';
  }
  this.traceLevel = traceLevel == '--quiet';
  ...
  this.trace = function (task, text) {
    if (this.traceLevel) console.log('[trace]'.bold() + '\x1b[0m ' + getTime().black().bold() + ' ' + task.purple() + ' ' + text);
  }
