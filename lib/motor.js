...
module.exports.DriveMotors = function (left, right, output) {
  ...
  this.check = function () {
    this.output.log('check', 'checking motors');
    (this.left.connected) ? this.output.info('check', 'left motor connected') : this.output.err('check', 'left motor is not connected');
    (this.right.connected) ? this.output.info('check', 'right motor connected') : this.output.err('check', 'right motor is not connected');
  }
}