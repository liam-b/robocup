...
module.exports.DriveMotors = function (left, right, output) {
  ...
  this.ratio = function (ratio, power) {
    if (this.left.connected && this.right.connected) {
      try {
        this.left.runForever(parseInt(ratio[0] * power));
      }
      catch (e) {
        this.output.err('ratio r', (ratio[0] * power).toString());
      }
      try {
        this.right.runForever(parseInt(ratio[1] * power));
      }
      catch (e) {
        this.output.err('ratio l', (ratio[1] * power).toString());
      }
    }
  }
