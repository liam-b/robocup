...
module.exports.PowerSupply = function (output) {
  ...
  this.check = function () {
    this.output.trace('check', 'checking battery ' + output.cyan(100 * ((this.battery.measuredVoltage - this.minVoltage) / (this.maxVoltage - this.minVoltage)));
    (100 * ((this.battery.measuredVoltage - this.minVoltage) / (this.maxVoltage - this.minVoltage)) > 30) ? this.output.log('check', 'battery ok') : this.output.warn('check', 'battery is low');
  }
}