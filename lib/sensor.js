...
module.exports.SeekerSensor = function (port, output) {
  ...
  this.value = function () {
    return (this.sensor.connected) ? [this.sensor.getValue(1), this.sensor.getValue(2), this.sensor.getValue(3), this.sensor.getValue(4), this.sensor.getValue(5), this._average()] : this.output.warn('value', 'seeker has been disconnected');
  }
  ...
  this._average = function () {
    return (this.sensor.getValue(1) + this.sensor.getValue(2) + this.sensor.getValue(3) + this.sensor.getValue(4) + this.sensor.getValue(5)) / array.length;
  }
}