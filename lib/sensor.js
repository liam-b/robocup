...
module.exports.CompassSensor = function (port, output) {
  ...
  this.mode = function (mode) {
    this.output.log('mode', 'changing compass sensor mode to ' + output.cyan(mode));
    (this.sensor.connected) ? this.sensor.setProperty('mode', mode) : this.output.err('mode', 'compass sensor is not connected');
  }
